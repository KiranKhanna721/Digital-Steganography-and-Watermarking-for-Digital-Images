...
def app():             
    ...
    classifiers =   st.sidebar.selectbox("Select Classifier",("LCB","DCT"))
...
# Decode the data in the image
    ...
    e_image = st.file_uploader("Choose an image...", type="jpg")
    submit = st.button('Encode')
    st.write("Submit",submit)
    message = st.text_input("Message","")
    if e_image is not None:
        if classifiers =='LCB':          
            en_image = encode(e_image,message)       
            lsb_hidden_text = decode(en_image)
            st.write(lsb_hidden_text)
        elif classifiers =='DCT':              
            dct_img_encoded = DCT().encode_image(e_image,message)     
            dct_hidden_text = DCT().decode_image(dct_img_encoded)
            st.write(dct_hidden_text)
